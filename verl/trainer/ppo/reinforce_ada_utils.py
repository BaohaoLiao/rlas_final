...
def ensure_uid_in_batch(batch: DataProto, context_batch: DataProto = None) -> None:
    ...
    raise KeyError(
        "batch missing uid and cannot copy from context_batch; "
        "ensure _get_gen_batch passes through uid"
    )
    ...
    if ctx_batch is not None:
        ...
        for k, v in ctx_batch.items():
            ...
            if v.shape[0] != n_selected:
                raise ValueError(
                    f"ctx_rows.batch['{k}'] row count({v.shape[0]}) != batch({n_selected})"
                )
            batch.batch[k] = v
...
def compute_seq_rewards_for_round(
    ...
    if Bg % Bp != 0:
        raise ValueError(
            f"Batch mismatch: gen_out({Bg}) is not a multiple of mini_prompt_batch({Bp})."
        )
    rep = Bg // Bp
    ...
    if "uid" not in gen_out.non_tensor_batch:
        if "uid" in mini_prompt_batch.non_tensor_batch:
            gen_out.non_tensor_batch["uid"] = np.repeat(
                np.array(mini_prompt_batch.non_tensor_batch["uid"], dtype=object),
                rep, axis=0
            )
        else:
    ...
    for k, v in mini_prompt_batch.non_tensor_batch.items():
        ...
        if arr.shape[0] != Bp:
            raise ValueError(
                f"mini_prompt_batch.non_tensor_batch['{k}'] length {arr.shape[0]} != {Bp}"
            )
    ...
    for key in required_keys:
        ...
        if miss == len(uids_round):
            raise KeyError(
                f"Required field '{key}' not found in mini_prompt_batch or context_batch"
            )
    ...
    if config.reward_model.launch_reward_fn_async:
        reward_tensor, reward_extra_infos_dict = ray.get(
            compute_reward_async.remote(data=mini, reward_fn=reward_fn)
        )
    else:
